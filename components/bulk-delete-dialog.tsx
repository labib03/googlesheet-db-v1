...
export function BulkDeleteDialog({
  ...
  const handleBulkDelete = async () => {
    ...
    startTransition(async () => {
      const result = await bulkDeleteData(selectedIndices);
      if (result.success) {
      ...
      <AlertDialogContent className="rounded-3xl border-none shadow-2xl p-0 overflow-hidden sm:max-w-[500px]">
        ...
        <div className="p-6 bg-white dark:bg-slate-900 space-y-4">
          <div className="space-y-2">
            <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Daftar nama yang dipilih:</p>
            <ScrollArea className="h-[150px] w-full rounded-xl border border-slate-100 dark:border-slate-800 p-3 bg-slate-50/50 dark:bg-slate-950/50">
