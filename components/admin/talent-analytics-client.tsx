        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            {mode !== "dashboard-only" && (
                <AnimatePresence mode="sync">
                    {isDiscoveryMode && (
                        <motion.div
                            // 1. Tambahkan prop layout agar Framer Motion menghitung ulang posisi secara otomatis
                            layout
                            ...
                            // 2. PERBAIKAN: Hapus 'p-6' dari sini dan pindahkan ke elemen anak (div interior)
                            className="bg-slate-100/50 dark:bg-slate-900/50 backdrop-blur-sm border border-slate-200/50 dark:border-slate-800/50 rounded-3xl overflow-hidden relative"
                        >
                            {/* 3. Padding diletakkan di sini agar tidak mengganggu kalkulasi height 0 */}
                                    ...
                                    <AnimatePresence mode="popLayout">
                                        {Object.entries(topDiscoveryTerms).length > 0 ? (
                                            Object.entries(topDiscoveryTerms).map(([term, count], index) => (
                                                <motion.div
                                                    key={term}
                                                    layout // Sangat penting agar keyword lain bergeser mulus saat satu keyword dihapus
                                                    initial={{ opacity: 0, scale: 0.8, y: 10 }}
                                                    animate={{
                                                        opacity: 1,
                                                        scale: 1,
                                                        y: 0,
                                                        transition: { delay: index * 0.02 } // Delay dikurangi sedikit agar lebih snappy
                                                    }}
                                                    exit={{
                                                        opacity: 0,
                                                        scale: 0.5,
                                                ...
                                                >
                                                    {/* Komponen Popover Anda */}
                                                    <Popover>
                                                        <PopoverTrigger asChild>
                                                            <button className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-indigo-400 dark:hover:border-indigo-600 hover:shadow-lg transition-all group">
